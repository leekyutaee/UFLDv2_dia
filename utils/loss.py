...
def soft_nll(pred, target, ignore_index = -1):
    ...
    C = pred.shape[1] #cls_out 의 1D 클래스 개수 가져옴 , 200
    invalid_target_index = target==ignore_index # 정답(label)이 무시 대상인 경우, True로 표시하는 마스크를 만듦 -1 은 true 로 -> 로스 계산할시 무시하기위해

    ttarget = target.clone() #원본회손 방비 클론만들기 , shape [4, 30, 10]
    ttarget[invalid_target_index] = C #-1으로 되어있는 값은 전주 C로 바꿔줌

    target_l = target - 1 #텐서 원소 전부 -1
    target_r = target + 1

    invalid_part_l = target_l == -1 #범위를 벚어났는지 확인
    invalid_part_r = target_r == C

    invalid_target_l_index = torch.logical_or(invalid_target_index, invalid_part_l) # 원래 target이 ignore 대상이거나 , target_l이 -1이면 → 그 위치는 무시하자.
    target_l[invalid_target_l_index] = C #무시할 친구는 C로 바꿔버림
    ...
    target_onehot = torch.nn.functional.one_hot(ttarget, num_classes=C+1) # shape [4, 30, 10, C+1]
    target_onehot = target_onehot[...,:-1].permute(0,3,1,2) # [4, 30, 10, C]
    ...
    target_fusion = 0.9 * target_onehot + 0.05 * target_l_onehot + 0.05 * target_r_onehot + 0.05 * supp_part_l_onehot + 0.05 * supp_part_r_onehot #shape , [4, C, 30, 10]
...
class SoftmaxFocalLoss(nn.Module):
    ...
    def forward(self, logits, labels): #logits = 예측값 , labels = 정답레이블
        scores = F.softmax(logits, dim=1) #dim=1을 기준으로 2,3차원은 softmax게산
        factor = torch.pow(1.-scores, self.gamma) #위와 같은 차원이지만 예측이 정확한 애들은 (score가 크니까) → factor가 작아지고, 틀릴 가능성이 높은 애들(score 작음) → factor가 커짐
        log_score = F.log_softmax(logits, dim=1)
        log_score = factor * log_score # "틀린 예측에 대해서는 log_score를 더 강조" 하는 거야.
        if self.soft_loss:
