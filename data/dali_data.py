...
class TrainCollect:
    ...
    dataset_name, top_crop):
        ...
        yy, xx = torch.meshgrid(self.interp_loc_row, self.interp_loc_col, indexing='ij')
        
        self.coords_flat = torch.stack((xx, yy), dim=2).reshape(-1, 2).contiguous().cuda()  # (2700, 2)
        self.dia_anchor_pos = torch.zeros(self.batch_size, 60, 30, 2) #[4][60][30][2]
        self.dia_anchor_neg = torch.zeros(self.batch_size, 60, 30, 2) #[4][60][30][2]
        ...
        for y in range(self.batch_size):
            for x in range(60):
                ...
                for i in range(30):
                    step = 89
                    self.dia_anchor_pos[y][x][i] = self.coords_flat[start_idx + (i) * step]
        ...
        for y in range(self.batch_size):
            for x in range(60):
                ...
                for i in range(30):
                    step = 91
                    self.dia_anchor_neg[y][x][i] = self.coords_flat[start_idx + (i) * step]
    def __iter__(self):
        return self

    def __next__(self):
        torch.set_printoptions(threshold=torch.inf, edgeitems=100, linewidth=200)
        ...
        # coords_flat = torch.stack((xx, yy), dim=2).reshape(-1, 2).contiguous().cuda()  # (2700, 2)
        # dia_anchor_pos = torch.zeros(self.batch_size, 60, 30, 2) #[4][60][30][2]
        # dia_anchor_neg = torch.zeros(self.batch_size, 60, 30, 2) #[4][60][30][2]
        ...
        #         for i in range(30):
        #             step = 89
        #             dia_anchor_pos[y][x][i] = coords_flat[start_idx + (i) * step]
        ...
        #         for i in range(30):
        #             step = 91
        #             dia_anchor_neg[y][x][i] = coords_flat[start_idx + (i) * step]
                    
        point_xy = my_interp.run(points,self.coords_flat) #([4, 2700, 2])
        
        anchor_pos = torch.zeros(self.batch_size, 60, 30, 2) #[4][60][30][2]
        anchor_neg = torch.zeros(self.batch_size, 60, 30, 2) #[4][60][30][2]
        ...
        for y in range(self.batch_size):
            for x in range(60):
                ...
                for i in range(30):
                    step = 89
                    anchor_pos[y][x][i] = point_xy[y][start_idx + (i) * step]
        ...
        for y in range(self.batch_size):
            for x in range(60):
                ...
                for i in range(30):
                    step = 91
                    anchor_neg[y][x][i] = point_xy[y][start_idx + (i) * step] #[4][60][30][2]
        anchor_len = 1201  # 대각선 길이 고정
        quant_pos_idx = torch.full((self.batch_size, 60, 30), -1, dtype=torch.long) #[4][60][30]
        quant_neg_idx = torch.full((self.batch_size, 60, 30), -1, dtype=torch.long) #[4][60][30]
             
        for y in range(self.batch_size):
            for x in range(60):
                anchor_line = self.dia_anchor_pos[y][x]  # [30, 2]
                start = anchor_line[0]
                end = anchor_line[29]
                direction = end - start  # 방향 벡터

                for i in range(30):
                    if (anchor_pos[y][x][i][0] < 0): continue
                    
                    vec = anchor_line[i] - start  # 시작점 기준 상대 위치 벡터
                    proj_len = torch.dot(vec, direction) / torch.norm(direction)  # projection 길이
                    #print(proj_len)
                    #print(anchor_len)
                    norm_pos = proj_len / anchor_len  # 0 ~ 1로 정규화
                    #print(norm_pos)
                    idx = (norm_pos * (self.num_cell_row - 1)).long()
                    if norm_pos < 0 or norm_pos > 1:
                        idx = -1  # 범위 밖은 무효 처리
                    quant_pos_idx[y][x][i] = idx
              
    # anchor_neg도 동일 방식
        for y in range(self.batch_size):
            for x in range(60):
                anchor_line = self.dia_anchor_neg[y][x]
                start = anchor_line[0]
                end = anchor_line[29]
                direction = end - start

                for i in range(30):
                    if (anchor_neg[y][x][i][0]<0): continue
                    
                    vec = anchor_line[i] - start
                    proj_len = torch.dot(vec, direction) / torch.norm(direction)
                    norm_pos = proj_len / anchor_len
                    idx = (norm_pos * (self.num_cell_row - 1)).long()
                    if norm_pos < 0 or norm_pos > 1:
                        idx = -1
                    quant_neg_idx[y][x][i] = idx
        ...
        labels_row_float = 0
        labels_col_float = 0
        # print("labels_row", quant_pos_idx.shape)
        # print("labels_col", quant_neg_idx.shape)
        return {'images':images, 'seg_images':seg_images, 'labels_row':quant_pos_idx, 'labels_col':quant_neg_idx, 'labels_row_float':labels_row_float, 'labels_col_float':labels_col_float}
